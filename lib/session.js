...
// No need for if else statement, just pass this.mute the this.muted boolean to determine volume
  checkForMute() {
    this.mute(this.muted);
  }

  // Use only one mute function and take advantage of this.muted and ternaries.
  mute(muted) {
    this.backgroundSong.volume = muted ? 0.0 : 0.2;
    this.fatality.volume =  muted ? 0.0 : 0.15;
    this.dino.jumpSound.volume =  muted ? 0.0 : 0.1;
    this.dino.gunshotSound.volume =  muted ? 0.0 : 0.3;
    this.dino.dryGunshotSound.volume =  muted ? 0.0 : 0.8;
    this.collision.cactusSplatSound.volume =  muted ? 0.0 : 0.9;
  }
}
